			...
			if(JOB_ROBOTICIST)
				...
				switch(backbag)
					...
					if(4)
						clothes_s.Blend(new /icon('icons/mob/clothing/back.dmi', "satchel"), ICON_OVERLAY)
			if(JOB_PRISONER)
				clothes_s = new /icon('icons/mob/clothing/under/color.dmi', "prison_s")
...
//		user << browse(HTML, "window=mob_occupation;size=[width]x[height]")
	var/datum/browser/popup = new(user, "mob_occupation", "<div align='center'>Occupation Preferences</div>", width + 100, height)
